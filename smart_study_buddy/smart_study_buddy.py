...
from llm_functions import load_data, split_text, initialize_llm, \
    generate_questions, create_retrieval_qa_chain

# Initialization of session states
# Since Streamlit always reruns the script when a widget changes, we need to
...
if uploaded_file is not None:
    ...
    if not openai_api_key:
        ...
        documents_for_question_gen = split_text(text_from_pdf,
                                                chunk_size=10000,
                                                chunk_overlap=200)

        # Split text for question answering
        documents_for_question_answering = split_text(text_from_pdf,
                                                      chunk_size=500,
                                                      chunk_overlap=200)

        # Initialize large language model for question generation
        llm_question_gen = initialize_llm(openai_api_key=openai_api_key,
                                          model="gpt-3.5-turbo-16k",
                                          temperature=0.4)

        # Initialize large language model for question answering
        llm_question_answering = initialize_llm(openai_api_key=openai_api_key,
                                                model="gpt-3.5-turbo",
                                                temperature=0.1)
        ...
        if st.session_state['questions'] == 'empty':
            with st.spinner("Generating questions..."):
                # Assign the generated questions to the session state.
                # This way, the questions are only generated once.
                st.session_state['questions'] = generate_questions(
                    llm=llm_question_gen, chain_type="refine",
                    documents=documents_for_question_gen)

        if st.session_state['questions'] != 'empty':
            # Show questions on screen.
            # You could use st.code for easy copy-pasting.
            st.info(st.session_state['questions'])

            # Split questions into a list
            st.session_state['questions_list'] = st.session_state[
                'questions'].split('\n')

            with st.form(key='my_form'):
                # Create a list of questions that have to be answered
                st.session_state['questions_to_answers'] = st.multiselect(
                    label="Select questions to answer",
                    options=st.session_state['questions_list'])
            ...
            if st.session_state['submitted']:
                ...
                with st.spinner("Generating answers..."):
                    generate_answer_chain = create_retrieval_qa_chain(
                        openai_api_key=openai_api_key,
                        documents=documents_for_question_answering,
                        llm=llm_question_answering)
